...
- (void)creatSystemMessage{
    ...
    [aString addAttributes:@{NSForegroundColorAttributeName:UIColorFromRGB(0xFF9600),NSFontAttributeName:[UIFont systemFontOfSize:14]} range:NSMakeRange(0, aString.length)];
    self.attributContext = aString;
}

- (void)creatComeInMessage{
    self.attributContext = [self creatNameAttributWithColon:NO];
    NSAttributedString *aString = [[NSAttributedString alloc]initWithString:@" 来了" attributes:@{NSForegroundColorAttributeName:UIColorFromRGB(0xffffff),NSFontAttributeName:[UIFont systemFontOfSize:14]}];
...
- (void)creatChatTextMessage{
    ...
    NSAttributedString *contextStr = [[NSAttributedString alloc]initWithString:self.msgText attributes:@{NSForegroundColorAttributeName:UIColorFromRGB(0xffffff),NSFontAttributeName:[UIFont systemFontOfSize:14]}];
    [_attributContext insertAttributedString:contextStr atIndex:_attributContext.length];
}

- (void)creatFollowMessage{
    self.attributContext = [self creatNameAttributWithColon:YES];
    NSAttributedString *contextStr = [[NSAttributedString alloc]initWithString:[NSString stringWithFormat:@" 关注了%@",self.toUserName ? self.toUserName : @"主播"] attributes:@{NSForegroundColorAttributeName:UIColorFromRGB(0xFF9600),NSFontAttributeName:[UIFont systemFontOfSize:14]}];
    [_attributContext insertAttributedString:contextStr atIndex:_attributContext.length];
}
...
- (NSMutableAttributedString*)creatNameAttributWithColon:(BOOL)colon{
    ...
    NSMutableAttributedString *nameAtr = [[NSMutableAttributedString alloc]initWithString:[NSString stringWithFormat:@" %@%@",self.fromUserName,colon ? @"：" : @""]attributes:@{NSForegroundColorAttributeName : UIColorFromRGB(0xFF9600),NSFontAttributeName : [UIFont systemFontOfSize:14]}];
...
- (void)creatContextSize{
    ...
    if (textSize.width < MessageMaxWidth -10) {
        size = textSize;
    }else{
        size = [self.attributContext boundingRectWithSize:CGSizeMake(MessageMaxWidth -10, 100000) options:NSStringDrawingUsesFontLeading | NSStringDrawingUsesLineFragmentOrigin context:nil].size;
    }
